...
public class BrowseClubHandler : BaseHandler<BrowseClubQuery, BrowseClubResponse>
{
    ...
    public override async Task<BrowseClubResponse> Handle(BrowseClubQuery request, CancellationToken cancellationToken)
    {
        ...
        if (queryResult.Count == 0)
        {
            ...
            var clubsDto = await GetClubsFromSpid(ObjectMapper.Map<BrowseClubQuery, SP_QUERY.BrowseClubsQuery>(request));
            if (clubsDto.Count > 0)
            {
                await PopulateClubDetail(clubsDto);
    ...
    protected async Task<List<ClubDto>> GetClubsFromSpid(SP_QUERY.BrowseClubsQuery query)
    {
    ...
    protected async Task PopulateClubDetail(List<ClubDto> clubs)
    {
        ...
        foreach (var club in clubs)
        {
            ...
            ObjectMapper.Map<ClubDetailDto, ClubDto>(club_detail_response, club);
